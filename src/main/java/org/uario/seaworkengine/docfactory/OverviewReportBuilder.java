...
public class OverviewReportBuilder {
	...
	public static JasperReportBuilder createReviewReport(final ArrayList<RowSchedule> list_row) {
		try {
			...
			// crate title
			report.title(OverviewReportBuilder.createTitleComponent());

			OverviewReportBuilder.build(report);

			final JRDataSource datasource = OverviewReportBuilder.createDataSource();
			// report.setDataSource(list_row);
			report.setDataSource(datasource);

			return report;
		} catch (final Exception e) {
			OverviewReportBuilder.logger.error("Error in creating report. " + e.getMessage());
			return null;
		}
	}
	...
	private static ComponentBuilder<?, ?> createTitleComponent() {
		...
		try {
			...
			final URL url = OverviewReportBuilder.class.getResource("/etc/reports/logo_report.png");
