...
public class ReviewReportBuilder {
	...
	private static Logger	logger	= Logger.getLogger(ReviewReportBuilder.class);
	...
	public static JasperReportBuilder createReport(final ArrayList<RowSchedule> list_row) {
		try {
			...
			// crate title
			report.title(ReviewReportBuilder.createTitleComponent());

			ReviewReportBuilder.build(report);

			final JRDataSource datasource = ReviewReportBuilder.createDataSource();
			// report.setDataSource(list_row);
			report.setDataSource(datasource);

			return report;
		} catch (final Exception e) {
			ReviewReportBuilder.logger.error("Error in creating report. " + e.getMessage());
			return null;
		}
	}
	...
	private static ComponentBuilder<?, ?> createTitleComponent() {
		...
		try {
			...
			final URL url = ReviewReportBuilder.class.getResource("/etc/reports/logo_report.png");
