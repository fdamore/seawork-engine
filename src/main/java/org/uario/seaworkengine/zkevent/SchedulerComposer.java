...
public class SchedulerComposer extends SelectorComposer<Component> {
	...
	private void assignShiftFromDayScheduler(final RowDaySchedule row_item, final UserShift shift, final int offset) {
		...
		if (daySchedule == null) {
			...
			if (this.shift_cache.getBreakShift() != null) {
				...
				if ((daySchedule.getShift() != null) && daySchedule.getShift().equals(this.shift_cache.getBreakShift().getId())) {
					return;
				}
	 ...
	 */
	@Listen("onChange = #scheduler_type_selector, #date_init_scheduler, #date_init_scheduler_review;onOK = #shows_rows, #full_text_search; onSelect = #overview_tab")
	...
	private void setOverviewLists() {
		if (this.overview_review.isSelected()) {
			...
			List<DetailFinalSchedule> list = null;

			if ((this.full_text_search.getValue() == null) || this.full_text_search.getValue().equals("")) {
				list = this.statisticDAO.listDetailFinalSchedule(null);
			}
			else {
				list = this.statisticDAO.listDetailFinalSchedule(this.full_text_search.getValue());
			}

			this.list_overview_review.setModel(new ListModelList<DetailFinalSchedule>(list));
			...
			List<DetailInitialSchedule> list = null;

			if ((this.full_text_search.getValue() == null) || this.full_text_search.getValue().equals("")) {
				list = this.statisticDAO.listDetailInitialSchedule(null);
			}
			else {
				list = this.statisticDAO.listDetailInitialSchedule(this.full_text_search.getValue());
			}

			this.list_overview_program.setModel(new ListModelList<DetailInitialSchedule>(list));
			if ((this.shows_rows.getValue() != null) && (this.shows_rows.getValue() != 0)) {
