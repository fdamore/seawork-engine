...
public class SchedulerComposer extends SelectorComposer<Component> {
	...
	public void addCompenstion() {
		...
		status_comp_editor = SchedulerComposer.STATUS_COMP_EDITOR_ADD;

		comp_editor.setVisible(true);
	}
	...
	public void commandCompenstion() {
		...
		if (user_statistic_selected == null) {
			return;
		}

		final Integer user_id = user_statistic_selected.getPerson().getId();
		final Integer year = Calendar.getInstance().get(Calendar.YEAR);

		if (status_comp_editor.equals(SchedulerComposer.STATUS_COMP_EDITOR_EDIT)) {

			if (sw_compensation_list.getSelectedItem() == null) {
				return;
			}

			final UserCompensation mod_item = sw_compensation_list.getSelectedItem().getValue();
			mod_item.setDate_submit(date_submitUser.getValue());
			mod_item.setId_user(user_id);
			mod_item.setNote(note_compUser.getValue());
			mod_item.setTime_comp(time_compUser.getValue());

			userCompensationDAO.updateUserCompensation(mod_item);

		} else if (status_comp_editor.equals(SchedulerComposer.STATUS_COMP_EDITOR_ADD)) {

			final UserCompensation new_item = new UserCompensation();
			new_item.setDate_submit(date_submitUser.getValue());
			new_item.setId_user(user_id);
			new_item.setNote(note_compUser.getValue());
			new_item.setTime_comp(time_compUser.getValue());

			userCompensationDAO.createUserCompensation(new_item);

		}

		final List<UserCompensation> list = userCompensationDAO.loadAllUserCompensationByUserId(user_id, year);
		final ListModelList<UserCompensation> model = new ListModelList<UserCompensation>(list);
		sw_compensation_list.setModel(model);

		refreshUserStatistics(full_text_search.getValue(), combo_user_dip.getValue());

		comp_editor.setVisible(false);
	}
	...
	public void deleteCompenstion() {
		...
		final UserCompensation user_comp = sw_compensation_list.getSelectedItem().getValue();

		userCompensationDAO.deleteUserCompensation(user_comp.getId());
	...
	public void doFinally() throws Exception {
		...
		getSelf().addEventListener(ZkEventsTag.onNameCompensationClick, new EventListener<Event>() {

			@Override
			public void onEvent(final Event arg0) throws Exception {

				if (list_statistics.getSelectedItem() == null) {
					return;
				}

				user_statistic_selected = list_statistics.getSelectedItem().getValue();
				...
				// set year label
				compe_popup_year.setLabel("" + Calendar.getInstance().get(Calendar.YEAR));

				compensation_popup.open(list_statistics, "after_pointer");
	...
	public void modCompenstion() {
		...
		date_submitUser.setValue(user_comp.getDate_submit());
		time_compUser.setValue(user_comp.getTime_comp());
		note_compUser.setValue(user_comp.getNote());

		status_comp_editor = SchedulerComposer.STATUS_COMP_EDITOR_EDIT;
	...
	private void refreshUserStatistics(final String text, final String department) {
		...
		for (final Person person : users_schedule) {
			...
			if (use_date) {
				user = statProcedure.getUserStatistics(person, date_from, date_to, false);
			} else {
				user = statProcedure.getUserStatistics(person, false);
			}
	...
	private void setOverviewLists(final Date date_from_overview, final Date date_to_overview) {
		...
		} else if (overview_statistics.isSelected()) {

			refreshUserStatistics(full_text_search, null);
		}
	}
	...
	private void showStatisticsPopup(final Integer id_user, final Component anchorComponent, final String title) {
		...
		final UserStatistics userStatistics = statProcedure.getUserStatistics(user, true);
