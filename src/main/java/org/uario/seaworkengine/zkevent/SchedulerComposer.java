...
public class SchedulerComposer extends SelectorComposer<Component> {
	...
	private void defineReviewByProgramProcedure() {
		...
		for (final Listitem item : this.grid_scheduler_review.getSelectedItems()) {
			...
			for (final DetailInitialSchedule init_item : list_init_detail) {
				...
				detail_schedule.setTime_to(init_item.getTime_from());
				detail_schedule.setTime_from(init_item.getTime_to());
	...
	public void doFinally() throws Exception {
		...
		this.getSelf().addEventListener(ZkEventsTag.onOverviewReviewNameClick, new EventListener<Event>() {
			...
			public void onEvent(final Event arg0) throws Exception {
				...
				final Integer id_user = SchedulerComposer.this.scheduleDAO.loadScheduleById(detailFinalSchedule.getId_schedule())
						.getUser();
				// set name
	...
	public void downloadReviewReport() {
		...
		try {
			...
			ReviewReportBuilder.createReviewReport(null).toPdf(stream);
	...
	/**
	 * Force assign random break
	 *
	 * @param date_scheduled
	 * @param user_id
	 */
	private void forceAssignRandomBreak(final Date date_scheduled, final Integer user_id) {
		...
		final Date date_break = SchedulerComposer.this.statProcedure.getARandomDay(cal_sunday.getTime(), 7);
		final Person person_logged = (Person) SecurityContextHolder.getContext().getAuthentication().getPrincipal();

		this.statProcedure.workAssignProcedure(SchedulerComposer.this.shift_cache.getBreakShift(), date_break, user_id,
				person_logged.getId());
		...
		if (dayAfterConfig != null) {
			...
			if ((current_schedule != null) && (current_schedule.getShift() != null)) {
				...
				if (shift_type.getBreak_shift().booleanValue() || shift_type.getWaitbreak_shift().booleanValue()) {
					...
					final List<Schedule> list_sch = this.statProcedure.searchBreakInCurrentWeek(
							current_schedule.getDate_schedule(), current_schedule.getUser());
					...
					if (list_sch == null) {
						...
						Messagebox.show(
								"Non ci sono riposi assegnati per qeusta settimana. Vuoi che ne assegni uno automaticamente?",
								"GESTIONE RIPOSI", buttons, null, Messagebox.EXCLAMATION, null, new EventListener<ClickEvent>() {
									...
									public void onEvent(final ClickEvent e) {
										if (Messagebox.ON_OK.equals(e.getName())) {
											...
											final Date date_break = SchedulerComposer.this.statProcedure.getARandomDay(
													cal.getTime(), 7);

											SchedulerComposer.this.statProcedure.workAssignProcedure(
													SchedulerComposer.this.shift_cache.getBreakShift(), date_break,
													current_schedule.getUser(), null);
			...
			if (sum != 0.0) {
				...
				if ((this.selectedShift.compareTo(min_shift) < 0) || (this.selectedShift.compareTo(max_shift) > 0)) {
					...
					Messagebox.show(msg, "CONFERMA CANCELLAZIONE", buttons, null, Messagebox.EXCLAMATION, null,
							new checkOnDoubleShiftBreaEvent(max_shift), params);
