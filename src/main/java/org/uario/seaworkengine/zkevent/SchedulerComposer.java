...
public class SchedulerComposer extends SelectorComposer<Component> {
	...
	private static final SimpleDateFormat formatDateOverview = new SimpleDateFormat(
			"dd/MM/yyyy");
	...
	private static final SimpleDateFormat formatter_eeee = new SimpleDateFormat(
			"EEEE");
	...
	private static final SimpleDateFormat formatter_scheduler_info = new SimpleDateFormat(
			"EEEE dd MMM");
	private static final SimpleDateFormat formatTimeOverview = new SimpleDateFormat(
			"dd/MM/yyyy hh:mm");
	...
	private final NumberFormat number_format = NumberFormat
	...
	public void assignShiftInPeriod() {
		...
		if (shift != null) {
			if (shift.getBreak_shift()) {
				...
				Messagebox
						.show("Non puoi usare il turno ti riposo per assegnazioni multiple.",
		...
		do {
			...
			// to do
			this.statProcedure.workAssignProcedure(shift,
					index_calendar.getTime(), user, editor.getId());
		...
		for (final Listitem item : this.grid_scheduler_review
			...
			for (final DetailInitialSchedule init_item : list_init_detail) {
				...
				case 1: {
					...
					detail_schedule.setTime_from(new Timestamp(to_day_calendar
							.getTimeInMillis()));
					to_day_calendar.set(Calendar.HOUR_OF_DAY, 7);
					detail_schedule.setTime_to(new Timestamp(to_day_calendar
							.getTimeInMillis()));
					break;
				}
				case 2: {
				...
				new EventListener<Event>() {
					...
					public void onEvent(final Event arg0) throws Exception {
						...
						SchedulerComposer.this.showStatisticsPopup(id_user,
								SchedulerComposer.this.grid_scheduler_day, msg);
		...
		this.getSelf().addEventListener(ZkEventsTag.onOverviewReviewNameClick,
				new EventListener<Event>() {
					...
					public void onEvent(final Event arg0) throws Exception {
						...
						final Integer id_user = SchedulerComposer.this.scheduleDAO
								.loadScheduleById(
										detailFinalSchedule.getId_schedule())
								.getUser();
						// set name
		...
		this.getSelf().addEventListener(ZkEventsTag.onOverviewProgramNameClick,
				new EventListener<Event>() {
					...
					public void onEvent(final Event arg0) throws Exception {
						...
						final Integer id_user = SchedulerComposer.this.scheduleDAO
								.loadScheduleById(
										detailInitialSchedule.getId_schedule())
								.getUser();
						// set name
				...
				new EventListener<Event>() {
					...
					public void onEvent(final Event arg0) throws Exception {
						...
						final List<UserShift> shifts = SchedulerComposer.this.configurationDAO
								.loadShifts();
						SchedulerComposer.this.shifts_combo_select
								.setModel(new ListModelList<UserShift>(shifts));

						// set combo in period shift composer
						SchedulerComposer.this.shift_period_combo
								.setModel(new ListModelList<UserShift>(shifts));

						// set info in force combo
						SchedulerComposer.this.force_shift_combo
								.setModel(new ListModelList<UserShift>(shifts));

						// set in overview
						SchedulerComposer.this.select_shifttype_overview
								.setModel(new ListModelList<UserShift>(shifts));

						// get the shift cache
						SchedulerComposer.this.shift_cache = (IShiftCache) SpringUtil
								.getBean(BeansTag.SHIFT_CACHE);
						SchedulerComposer.this.task_cache = (ITaskCache) SpringUtil
								.getBean(BeansTag.TASK_CACHE);
		...
		if (this.overview_review.isSelected()
			...
			for (final DetailFinalSchedule item : this.listDetailRevision) {
				...
				if (item.getDate_schedule() != null) {
					date = SchedulerComposer.formatDateOverview.format(item
							.getDate_schedule());
				}

				String time_from = "";
				if (item.getTime_from() != null) {
					time_from = SchedulerComposer.formatTimeOverview
							.format(item.getTime_from());
				}

				String time_to = "";
				if (item.getTime_to() != null) {
					time_to = SchedulerComposer.formatTimeOverview.format(item
							.getTime_to());
				}

				String code_task = "";
				final UserTask task = this.task_cache.getUserTask(item
						.getTask());
				if (task != null) {
				...
				String code_shift = "";
				final UserShift shift_type = this.shift_cache.getUserShift(item
						.getShift_type());
				if (shift_type != null) {
				...
				final String line = "" + item.getUser() + ";" + date + ";"
						+ code_shift + ";" + shift_no_info + ";" + code_task
						+ ";" + time_info + ";" + time_from + ";" + time_to
						+ ";\n";
				builder.append(line);
			}
			...
			for (final Schedule item : this.listSchedule) {
				...
				String code_shift = "";
				final UserShift task = this.shift_cache.getUserShift(item
						.getShift());
				if (task != null) {
					code_shift = task.getCode();
				}

				final String line = "" + item.getName_user() + ";" + date + ";"
						+ code_shift + ";\n";
				builder.append(line);
			}
	 ...
	 * @param schedule
	 */
	private ItemRowSchedule getItemRowSchedule(final RowSchedule currentRow,
		...
		if (dayScheduleDate != null) {
			...
			if (dayAfterConfig != null) {
				...
				if ((this.selectedDay + count) > SchedulerComposer.DAYS_IN_GRID_PREPROCESSING) {
					...
					Messagebox
							.show("Non puoi programmare oltre i limiti della griglia corrente",
									"ERROR", buttons, null,
				...
				this.scheduleDAO.removeScheduleUserSuspended(
						row_item.getUser(), dayScheduleDate, dayAfterConfig);
				...
				this.scheduleDAO.removeScheduleUserSuspended(
						row_item.getUser(), dayScheduleDate, dayScheduleDate);

				// check for tomorrow
				if (DateUtils
						.isSameDay(tomorrow_cal.getTime(), dayScheduleDate)) {
					final Schedule schedule = new Schedule();
					this.assignShiftForDaySchedule(shiftStandard,
							tomorrow_cal.getTime(), row_item.getUser(),
							schedule, person_logged, null);
	...
	public void saveDayScheduling() {
		...
		if (this.day_after_config.getValue() == null) {
			...
			if (shift.getBreak_shift().booleanValue()) {
				...
				Messagebox
						.show("Non puoi usare il turno di riposo programmato per assegnazioni multiple.",
									...
									new EventListener() {
										...
										public void onEvent(final Event e) {
											if (Messagebox.ON_OK.equals(e
												...
												// close popup
												SchedulerComposer.this.shift_definition_popup
														.close();
	...
	private void setGridStructureForShift() {
		...
		for (int i = 0; i < SchedulerComposer.DAYS_IN_GRID_PROGRAM; i++) {
			...
			final Auxheader week_head = (Auxheader) this.getSelf()
					.getFellowIfAny("day_week_" + index_day);
			...
			final String day_w = SchedulerComposer.formatter_eeee
					.format(current_calendar.getTime());
	...
	private void setGridStructureForShiftReview(final Date initial_date) {
		...
		for (int i = 0; i < SchedulerComposer.DAYS_TO_SHOW_IN_REVIEW; i++) {
			...
			final Auxheader week_head = (Auxheader) this.getSelf()
					.getFellowIfAny("day_week_review_" + index_day);
			if ((month_head == null) || (week_head == null)) {
				continue;
			}
	...
	private void setupGlobalSchedulerGridForShift() {
		...
		Double[][] count_matrix = new Double[5][4];		
		...
		// count review
		for (RowSchedule itemrow_count : list_row) {
		...
		final List<Person> users_schedule = this.personDAO
				.listWorkerPersons(text_search_person);
	...
	private void showStatisticsPopup(final Integer id_user,
		...
		// SET WORK SUNDAY
		final Double perc = SchedulerComposer.this.statisticDAO
				.getSundayWorkPercentage(id_user);
		String perc_info = "0%";
		if (perc != null) {
		...
		final Double perc_holidays = SchedulerComposer.this.statisticDAO
				.getSundayAndHolidaysWorkPercentage(id_user);
		...
		final RateShift[] statistic = SchedulerComposer.this.statisticDAO
				.getAverageForShift(id_user, c.getTime());
		...
		Integer month_current_hours = this.statisticDAO.getTimeWorked(id_user,
				date_from, date_to);
		...
		final Integer day_series = this.statProcedure.getWorkingSeries(
				take_tioday.getTime(), id_user);
		String message = "0";
		if (day_series <= 15) {
			message = "" + day_series;
		} else {
