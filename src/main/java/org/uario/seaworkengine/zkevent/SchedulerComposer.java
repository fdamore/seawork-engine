...
public class SchedulerComposer extends SelectorComposer<Component> {
	...
	private void definingPrintableVersion() {
		...
		grid_scheduler.setWidth("950px");
		grid_scheduler_review.setWidth("950px");
	...
	public void reprogramUser() {
		...
		for (final Listitem item : grid_scheduler.getSelectedItems()) {
			...
			statProcedure.workAssignProcedure(shift, date_tomorrow, person.getId(), person_logged.getId());
	...
	private void setOverviewLists() {
		...
		// select date
		if (date_from_overview.getValue() != null) {
			date_from = date_from_overview.getValue();
		}
		if (date_to_overview.getValue() != null) {
			date_to = date_to_overview.getValue();
		}
		if ((date_from == null) && (date_to != null)) {
			date_from = date_to;
		}
	...
	private void setupGlobalSchedulerGridForShift() {
		...
		final List<Person> users_schedule = personDAO.listWorkerPersons(text_search_person);
	...
	private void showStatisticsPopup(final Integer id_user, final Component anchorComponent, final String title) {
		...
		// set perc
		work_sunday_perc.setValue(perc_info);
		...
		// set perc
		work_sundayandholiday_perc.setValue(perc_info_holidays);
		...
		if (statistic != null) {
			for (final RateShift av : statistic) {
				...
				if (av.getShift() == 1) {
					SchedulerComposer.this.shift_perc_1.setValue(shift_perc_1.getValue() + " (" + Utility.roundTwo(av.getRate())
							+ "%)");
				}
				if (av.getShift() == 2) {
					SchedulerComposer.this.shift_perc_2.setValue(shift_perc_2.getValue() + " (" + Utility.roundTwo(av.getRate())
							+ "%)");
				}
				if (av.getShift() == 3) {
					SchedulerComposer.this.shift_perc_3.setValue(shift_perc_3.getValue() + " (" + Utility.roundTwo(av.getRate())
							+ "%)");
				}
				if (av.getShift() == 4) {
					SchedulerComposer.this.shift_perc_4.setValue(shift_perc_4.getValue() + " (" + Utility.roundTwo(av.getRate())
							+ "%)");
				}
			}
		}
		...
		Integer week_current_hours = statisticDAO.getTimeWorked(id_user, date_from, date_to);
		if (week_current_hours == null) {
			week_current_hours = 0;
		}

		work_current_week.setValue("" + week_current_hours);
		...
		Integer month_current_hours = statisticDAO.getTimeWorked(id_user, date_from, date_to);
		if (month_current_hours == null) {
			month_current_hours = 0;
		}

		work_current_month.setValue("" + month_current_hours);
		...
		Integer year_current_hours = statisticDAO.getTimeWorked(id_user, date_from, date_to);
		if (year_current_hours == null) {
			year_current_hours = 0;
		}

		work_current_year.setValue("" + year_current_hours);

		// working series
		final Calendar take_today = Calendar.getInstance();
		final Integer day_series = statProcedure.getWorkingSeries(take_today.getTime(), id_user);
		String message = "0";
		if (day_series <= 15) {
			message = "" + day_series;
		} else {
		...
		working_series.setValue(message);
