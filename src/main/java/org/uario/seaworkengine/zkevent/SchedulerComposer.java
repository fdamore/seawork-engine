...
public class SchedulerComposer extends SelectorComposer<Component> {
	...
	public void downdoadOverviewCSV() {
		...
		if (this.overview_review.isSelected() && (this.listDetailRevision != null)) {
			...
			for (final DetailFinalSchedule item : this.listDetailRevision) {
				...
				final String time_vacation_info = this.number_format.format(time_vacation);
	 ...
	 */
	private void saveDaySchedulingReplaceBreakShift(final UserShift shift, final RowDaySchedule row_item,
			final Date date_scheduled, final Boolean replace, final List<Schedule> scheduleListInWeek) {
		...
		if (this.define_program_body.isVisible()) {
			...
			if (sum != 0.0) {
				...
				if ((this.selectedShift.compareTo(min_shift) < 0) || (this.selectedShift.compareTo(max_shift) > 0)) {
					...
					Messagebox.show(
							"Stai assegnando un turno prima che ne siano passati 2 di stacco. Sei sicuro di voler continuare?",
						...
						new EventListener() {
							...
							public void onEvent(final Event e) {
								...
								if (Messagebox.ON_OK.equals(e.getName())) {
									...
									SchedulerComposer.this.saveDaySchedulingReplaceBreakShift(shift, row_item, date_scheduled,
											true, scheduleListInWeek);
