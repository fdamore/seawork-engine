...
import org.uario.seaworkengine.zkevent.bean.MonitorDataStructure;
...
public class ShipSchedulerComposer extends SelectorComposer<Component> {
	...
	@Wire
	private Listbox											list_monitor_detail;

	@Wire
	private Listbox											list_monitor_detail_ship;
	...
	@Wire
	private Div												monitor_detail;

	@Wire
	private Div												monitor_detail_ship;
		...
		if ((list != null) && (list.size() != 0)) {
			...
			for (final DetailScheduleShip itm_details : list) {
				...
				if (!(itm_details.getId_ship() == null)) {
					...
					final Ship ship = this.shipDao.loadShip(itm_details.getId_ship());
	...
	public void checkShiftDate_detail() {
		...
		if ((this.detailScheduleShipSelected != null) && (ShipSchedulerComposer.this.shiftdate_Daily.getValue() != null)
								&& ((ShipSchedulerComposer.this.shiftdate_Daily.getValue()
														...
														|| (ShipSchedulerComposer.this.shiftdate_Daily.getValue()
																				.compareTo(ShipSchedulerComposer.this.detailScheduleShipSelected
																										.getDeparturedate()) > 0))) {
	...
	public Listbox getList_monitor_detail_ship() {
		return this.list_monitor_detail_ship;
	}

	public Div getMonitor_detail() {
		return this.monitor_detail;
	}
	...
	@Listen("onSelect = #list_monitor")
	public void onSelectMonitor() {
		this.monitor_detail.setVisible(false);
		this.monitor_detail_ship.setVisible(false);

	}

	@Listen("onSelect = #list_monitor_detail")
	public void onSelectMonitorDetail() {
		this.monitor_detail_ship.setVisible(false);
	}
		...
		if (StringUtils.isEmpty(rifMCT) && (this.text_search_rifSWS.getValue() == null)) {
			...
			if (!this.no_invoice.isChecked()) {
				this.list_details_programmed_ship = this.shipSchedulerDao.searchDetailScheduleShip(dateFrom, dateTo, date_shift, period_on_dateshift,
										text_search, no_shift, idCustomer, nowork, activityh, worked, idServiceSelected, ship_type, ship_line,
										ship_condition, type_operation, invoice_period);
			} else {
				this.list_details_programmed_ship = this.shipSchedulerDao.searchDetailScheduleShip(dateFrom, dateTo, date_shift, period_on_dateshift,
										text_search, no_shift, idCustomer, nowork, activityh, worked, idServiceSelected, ship_type, ship_line,
										ship_condition, type_operation, invoice_period, true);
			}
	...
	public void setList_monitor_detail_ship(final Listbox list_monitor_detail_ship) {
		this.list_monitor_detail_ship = list_monitor_detail_ship;
	}

	public void setMonitor_detail(final Div monitor_detail) {
		this.monitor_detail = monitor_detail;
	}

	public void setMonitor_detail_ship(final Div monitor_detail_ship) {
		this.monitor_detail_ship = monitor_detail_ship;
	}
	...
	@Listen("onClick = #showMonitorDetail")
	...
	public void showMonitorShipDetail() {
		...
		// check map for sws
		final HashMap<Integer, Integer> check_map = new HashMap<>();

		for (final Listitem itm : list) {

			final DetailFinalSchedule itm_str = itm.getValue();

			final Integer rif_sws = itm_str.getRif_sws();
			if (rif_sws == null) {
				continue;
			}

			if (check_map.containsKey(rif_sws)) {
				continue;
			}

			final List<DetailScheduleShip> list_detail = this.shipSchedulerDao.loadDetailScheduleShipByIdSchedule(rif_sws);

			// put in check map
			check_map.put(rif_sws, rif_sws);
	...
	public void updateDataMonitor() {
		...
		for (final String key : map.keySet()) {
			...
			itm_str.setShift_no(first.getShift_no());

			final ArrayList<String> crane_list = new ArrayList<>();
			...
			for (final MonitorData mnt : itm_list) {
				...
				// set crane
				final String current_crane = mnt.getCrane();
				if (current_crane != null) {
					final String[] info = current_crane.split(",");
					for (final String cr : info) {
						final String cr_trimmed = cr.trim();
						if (!crane_list.contains(cr_trimmed)) {
							crane_list.add(cr_trimmed);
						}
					}
				}
			...
			// define crane for structured monitor
			final StringBuilder builder = new StringBuilder();

			for (int i = 0; i < crane_list.size(); i++) {

				final String cr = crane_list.get(i);
			...
			itm_str.setCrane(builder.toString());
			list_final.add(itm_str);
